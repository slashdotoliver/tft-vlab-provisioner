-- ==============================================================================
-- FUNCIONES DE UTILIDAD (TRIGGERS PARA UPDATED_AT)
-- ==============================================================================
...
CREATE TYPE lease_actual_vm_state AS ENUM ('pending', 'starting', 'running', 'terminating_by_user', 'terminating', 'terminated', 'terminated_by_user', 'error');
...
CREATE TABLE projects (
    ...
    CONSTRAINT pk_projects_owners FOREIGN KEY (owner_id) REFERENCES users(id) ON DELETE RESTRICT
);
...
-- ==============================================================================
-- TABLAS DE CATÁLOGO Y POLÍTICAS
-- ==============================================================================
...
CREATE TABLE template_networks (
    id UUID DEFAULT uuidv7(),
    template_id UUID NOT NULL,
    network_type VARCHAR(50) NOT NULL,
    bridge_name VARCHAR(50) NOT NULL,
    CONSTRAINT pk_template_networks PRIMARY KEY (id),
...
CREATE TABLE policies (
    ...
    max_lease_minutes INTEGER NOT NULL,
    min_advance_minutes INTEGER NOT NULL,
    allow_on_demand BOOLEAN DEFAULT TRUE,
    demand_capacity_buffer_mb INTEGER DEFAULT 0,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT pk_policies PRIMARY KEY (id),
    CONSTRAINT fk_policies_tenants FOREIGN KEY (tenant_id) REFERENCES projects(id) ON DELETE CASCADE,
    CONSTRAINT chk_policies_max_lease CHECK (max_lease_minutes > 0),
    CONSTRAINT chk_policies_min_adv CHECK (min_advance_minutes >= 0)
);
...
CREATE TABLE nodes (
    ...
    is_active BOOLEAN DEFAULT TRUE,
    allocated_vcpus INTEGER DEFAULT 0,
    allocated_ram_mb INTEGER DEFAULT 0,
    allocated_disk_gb INTEGER DEFAULT 0,
...
CREATE TABLE leases (
    ...
    -- CONSTRAINT chk_leases_times CHECK (end_time > start_time)
);
...
-- ==============================================================================
-- ÍNDICES DE OPTIMIZACIÓN Y CLAVES FORÁNEAS
-- ==============================================================================

CREATE INDEX idx_users_role_id ON users(role_id);
CREATE INDEX idx_project_users_project_id ON project_users(project_id);
CREATE INDEX idx_template_disks_template_id ON template_disks(template_id);
CREATE INDEX idx_template_networks_template_id ON template_networks(template_id);
...
CREATE INDEX idx_leases_time_range ON leases(start_time, end_time); -- TODO: change index type
...
-- ==============================================================================
-- NOTIFICACIONES EN TIEMPO REAL (PUB/SUB)
-- ==============================================================================
