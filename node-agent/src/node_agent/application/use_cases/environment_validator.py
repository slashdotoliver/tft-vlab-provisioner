...
from node_agent.application.ports.enviroment_provider import SystemEnvironmentPort, LibvirtEnvironmentPort
from node_agent.domain.model.environment_models import EnvironmentConfig, ValidationReport, EnvironmentCheckError, \
    ValidationStepResult, GuestSupport, PoolSupport, PoolCapability
from node_agent.domain.model.result import Result, Failure
...
class EnvironmentValidator:
    ...
    def validate_environment(self, config: EnvironmentConfig) -> Result[ValidationReport, EnvironmentCheckError]:
        ...
        def _report_failure_of(step: ValidationStepResult, rep: ValidationReport) -> None | Failure[EnvironmentCheckError]:
            if step.is_failure():
                return Result.failure(step.get_error())
            if issue := step.value_or(None): rep.add_issue(issue)
            return None

        for validation_step in [
            # system
            lambda: self._check_architecture(config.required_arch),
            lambda: self._check_hw_virt() if config.requires_hw_virt else Result.success(None),
            lambda: self._check_nested_virt() if config.requires_nested_virt else Result.success(None),
    ...
    def _check_libvirt_guest_capabilities(self, required_guest_support: list[GuestSupport]) -> ValidationStepResult:
        ...
        if missing:
            return Result.success(
                f"Missing required guest support: {', '.join(map(lambda m: str(m), missing))}. Available: {', '.join(map(lambda s: str(s), supported))}"
            )
        return Result.success(None)
    ...
    def _check_libvirt_pool_capabilities(self, required_pool_support: list[PoolSupport]) -> ValidationStepResult:
        def _get_missing_details(required: PoolSupport) -> str | None:
            ...
            if not supported_pool_cap:
                return f"'{required.pool_type}' (completely missing)"
            supported_pool: PoolSupport = PoolSupport(
                supported_pool_cap.pool_type,
                supported_pool_cap.source_formats,
                supported_pool_cap.target_formats
            )
            ...
            if not missing_sources and not missing_targets:
                return None
            errs = [f"{k}: {', '.join(v)}" for k, v in [("sources", missing_sources), ("targets", missing_targets)] if
                    v]
